...
struct PrivacyAppDetail: View {
  ...
  @State var displayReportPrivacyElements: [DPNDPrivacyAccess] = []
  ...
  var body: some View {
    VStack {
      ...
      List {
        Section(header:
          ...
          if !folderPrivacys {
            if displayReportPrivacyElements.count > 0 {
              ForEach(displayReportPrivacyElements, id: \.self) { privacy in
                HStack(spacing: 8) {
                  Circle()
                    .foregroundColor(.red)
    ...
    .onAppear {
      handlesReportPrivacyElements()
    }
  }
  
  func handlesReportPrivacyElements() {
    displayReportPrivacyElements.removeAll()
    for e in app.reportPrivacyElement {
      if let index = displayReportPrivacyElements.firstIndex(where: {$0.identifier == e.identifier}) {
        // 包含
        if e.kind == "intervalBegin"{
          displayReportPrivacyElements[index] = DPNDPrivacyAccess(accessor: e.accessor, category: e.category, identifier: e.identifier, beginTimeStamp: e.timeStamp, endTimeStamp: displayReportPrivacyElements[index].endTimeStamp, type: e.type)
        } else {
          displayReportPrivacyElements[index] = DPNDPrivacyAccess(accessor: e.accessor, category: e.category, identifier: e.identifier, beginTimeStamp: displayReportPrivacyElements[index].beginTimeStamp, endTimeStamp: e.timeStamp, type: e.type)
        }
        
      } else {
        var tmpElement: DPNDPrivacyAccess
        if e.kind == "intervalBegin"{
          tmpElement = DPNDPrivacyAccess(accessor: e.accessor, category: e.category, identifier: e.identifier, beginTimeStamp: e.timeStamp, endTimeStamp: "", type: e.type)
        } else {
          tmpElement = DPNDPrivacyAccess(accessor: e.accessor, category: e.category, identifier: e.identifier, beginTimeStamp: "", endTimeStamp: e.timeStamp, type: e.type)
        }
        displayReportPrivacyElements.append(tmpElement)
      }
    }
  }
